...
public class SpiroUtilTest {
	...
	/**
	 * Some pre-computed values to test the methods hypoX, hypoY, and hypo;
	 * these values come from a reference implementation that is thought to be
	 * correct.
	 */
	...
	private static final double[] X = { 0.8800000000000001, 0.6927514236751531, -0.45387130321761343,
			0.5507109777662673 };
	private static final double[] Y = { 0.0, 0.09134533712962353, -0.009249016917054598, 0.2949760093180739 };
	...
	/**
	 * Test that hypoX() throws and exception when using illegal values for the
	 * small wheel radius and/or pencil radius.
	 * 
	 * <p>
	 * Test that hypoX() does not throw an exception when using legal values for
	 * the small wheel radius and/or pencil radius.
	 */
	@Test
	...
	/**
	 * Test that hypoY() throws and exception when using illegal values for the
	 * small wheel radius and/or pencil radius.
	 * 
	 * <p>
	 * Test that hypoY() does not throw an exception when using legal values for
	 * the small wheel radius and/or pencil radius.
	 */
	@Test
	...
	/**
	 * Test that hypo() throws and exception when using illegal values for the
	 * small wheel radius and/or pencil radius.
	 * 
	 * <p>
	 * Test that hypo() does not throw an exception when using legal values for
	 * the small wheel radius and/or pencil radius.
	 */
	@Test
	...
	public void test04_hypoX() {
		for (int i = 0; i < SpiroUtilTest.DEGREES.length; i++) {
			...
			double got = SpiroUtil.hypoX(SpiroUtilTest.WHEEL_RADIUS, SpiroUtilTest.PENCIL_RADIUS, deg);
			String error = String.format("hypoX(%s, %s, %s) failed\n", SpiroUtilTest.WHEEL_RADIUS,
					SpiroUtilTest.PENCIL_RADIUS, deg);
	...
	public void test05_hypoY() {
		for (int i = 0; i < SpiroUtilTest.DEGREES.length; i++) {
			...
			double got = SpiroUtil.hypoY(SpiroUtilTest.WHEEL_RADIUS, SpiroUtilTest.PENCIL_RADIUS, deg);
	...
	public void test06_hypo() {
		for (int i = 0; i < SpiroUtilTest.DEGREES.length; i++) {
			...
			Point2 got = SpiroUtil.hypo(SpiroUtilTest.WHEEL_RADIUS, SpiroUtilTest.PENCIL_RADIUS, deg);

			String error = String.format("hypo(%s, %s, %s) failed\n" + "the x-coordinate was wrong\n",
					SpiroUtilTest.WHEEL_RADIUS, SpiroUtilTest.PENCIL_RADIUS, deg);
