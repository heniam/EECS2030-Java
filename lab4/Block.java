...
public abstract class Block {
    ...
    public void moveLeft() {
    	 this.position = new Point2(this.position.getX() - 1.0, this.position.getY());
        // this.draw();
    }
    /**
    ...
    public void moveDown() {
    	this.position = new Point2(this.position.getX(), this.position.getY() - 1.0);
        this.draw();
    }
     ...
     */
    public void moveUp() {
    	this.position = new Point2(this.position.getX(), this.position.getY());
    }
    ...
    public Point2 getPosition() {
    	  ...
    	  for (int i = 0; i < this.grid.size(); i++) {
              for (int j = 0; j < this.grid.size(); j++) {
                  if (this.grid.get(i, j)) {
                      Point2 topLeft = new Point2(this.position.getX() + j, this.position.getY() - i);
                      StdDraw.filledSquare(topLeft.getX() + 0, topLeft.getY() - 0, 0);
                  }
              }
          }
		return position;
