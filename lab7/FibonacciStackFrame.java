...
public class FibonacciStackFrame {
	...
	public void returnValueToCaller(Stack<FibonacciStackFrame> callStack) {
		if (this.caller == null) {
			...
			this.caller.receiveReturnValue(getReturnValue());
			callStack.pop();
		}
	...
	public void execute(Stack<FibonacciStackFrame> callStack) {
		...
		int value = callStack.peek().getN();
		if (value == 0) {
			this.sum = BigInteger.ZERO;
			returnValueToCaller(callStack);
		} else if (value == 1) {
			this.sum = BigInteger.ONE;
			returnValueToCaller(callStack);
		} else if (cache.containsKey(value)) {
			this.sum = cache.get(n);
			returnValueToCaller(callStack);
		} else if (callStack.peek().fMinus1Computed == false || callStack.peek().fMinus2Computed == false) {
			FibonacciStackFrame current = callStack.peek();
			if (current.fMinus1Computed == false) {

				FibonacciStackFrame obj = new FibonacciStackFrame(n - 1, this);
				callStack.push(obj);

			} else if (current.fMinus2Computed == false) {

				FibonacciStackFrame obj = new FibonacciStackFrame(n - 2, this);
				callStack.push(obj);
			}
		} else {
			this.sum = getfMinus1().add(getfMinus2());
